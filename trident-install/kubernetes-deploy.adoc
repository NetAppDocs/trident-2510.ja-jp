...
適切なインストール方法を選択してください。決定を下す前にlink:kubernetes-deploy.html#moving-between-installation-methods["方法間の移動"]の考慮事項も確認する必要があります。



=== Tridentオペレーターの使用

手動でのデプロイでもHelmを使用する場合でも、Tridentオペレーターはインストールを簡素化し、Tridentリソースを動的に管理する優れた方法です。さらに、link:../trident-install/kubernetes-customize-deploy.html["Tridentオペレータの導入をカスタマイズ"]は、 `TridentOrchestrator`カスタムリソース（CR）の属性を使用して行うこともできます。
