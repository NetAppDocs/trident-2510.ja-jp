...
.手順
. Trident Protect CLIプラグインを使用して、デスティネーション クラスタにAppVault CRが存在することを確認します：
+
[source, console]
----
...
NOTE: AppVault CR がデスティネーション クラスタに存在しない場合は、link:trident-protect-appvault-custom-resources.html["Trident Protect AppVaultオブジェクトを使用してバケットを管理する"]の手順に従って作成します。

. デスティネーション クラスタで利用可能なAppVaultのバックアップコンテンツを表示し、復元するバックアップの `appArchivePath`をメモします：
+
[source, console]
----
...
+-------------+-----------+--------+-----------------+--------------------------+-------------+
----
. AppVault 名とアーカイブパスを使用して、デスティネーション クラスタにアプリケーションを復元します：注：CRを使用する場合は、アプリケーションの復元に使用する名前空間がデスティネーション クラスタに存在することを確認してください。
